# Reto_13
### 1. El siguiente código contiene un JSON con el pronostivo detallado del clima para 8 días:
...
### Solucion
Este punto en especifico se me hizo demasiado díficil...

```Python
import json
from datetime import datetime

def procesar_alerta(data, tipo_alerta, nombre_alerta, clave_descripcion, clave_valor, unidades=''):
    for key, valores in data[tipo_alerta].items():
        if valores == "X":
            print(f"¡ALERTA POR {nombre_alerta}!")
            print(f"Tipo de alerta: {nombre_alerta} {valores}")
            ...
            descripcion = data[clave_descripcion].get(key)
            print(f"{descripcion}: {data[clave_valor].get(key)} {unidades}")
...
if __name__ == "__main__":
    ...
    procesar_alerta(data, "alertAlertas", "ALERTAS", "description", "main")
    procesar_alerta(data, "alertPrecip", "PRECIPITACIONES", "description", "prcp", "mm")
    procesar_alerta(data, "alertTmpMax", "TEMPERATURA MÁXIMA", "description", "tmpMax", "°C")
    procesar_alerta(data, "alertTmpMin", "TEMPERATURA MÍNIMA", "description", "tmpMin", "°C")
    procesar_alerta(data, "alertVelViento", "VELOCIDAD DEL VIENTO", "dirViento", "velViento", "m/s")
```
### 2. A través de un programa conectese a al menos 3 [API's ](https://apipheny.io/free-api/), obtenga el JSON, imprimalo y extraiga los pares de llave : valor.
